...
export default async function AdminUsersPage() {
  ...
  try {
    ...
    if (process.env.FIREBASE_ADMIN_PROJECT_ID) {
      const [usersSnapshot, counsellorsSnap] = await Promise.all([
        adminDb.collection('users').orderBy('createdAt', 'desc').limit(50).get(),
        adminDb.collection('users').where('role', '==', 'counsellor').get(),
      ]);

      const counsellorMap = new Map<string, { name: string; email: string; mobile: string; designation: string }>();
      counsellorsSnap.docs.forEach(cDoc => {
        const cData = cDoc.data();
        counsellorMap.set(cDoc.id, {
          name: cData.name || 'Counsellor',
          email: cData.email || '',
          mobile: cData.mobile || '',
          designation: cData.designation || 'Education Counsellor',
        });
      });
      ...
      initialUsers = usersSnapshot.docs.map(doc => {
        ...
        const assignedCId = data.assignedCounsellor;
        const cInfo = assignedCId ? counsellorMap.get(assignedCId) : null;
        ...
        return {
          ...
          counsellorName: cInfo ? cInfo.name : 'Not Assigned',
          counsellorEmail: cInfo ? cInfo.email : '',
          counsellorMobile: cInfo ? cInfo.mobile : '',
          counsellorDesignation: cInfo ? cInfo.designation : '',
